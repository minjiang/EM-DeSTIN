...
Destin * InitDestin( uint ni, uint nl, uint *nb, uint nc, float beta, float lambda, float gamma, float *temp, float starvCoeff, uint nMovements, bool isUniform, int extRatio)
{
	...
	MALLOC(d->nDim, uint, n);
    	MALLOC(d->nPat, uint, n);
	uint m = 0;
    ...
    //分配足够的raw memory，给一个预备容纳n个cEm objects的数组使用
	void *rawMemory = operator new(n*sizeof(ker::cEm));
	
   	//让bestPieces指向此内存，使这块内存被视为一个cEm数组
	d->T = reinterpret_cast<ker::cEm*>(rawMemory);
	
	//利用"placement new"构造这块内存中的cEm objects。
	for(int i = 0; i < n;i++){
		new (&d->T[i]) ker::cEm( d->nDim[i], d->nPat[i]);
	}
	//for(int i = 0; i < m;i++){
	//d->T[0].ls();}
//------------------------2013.11.13-----------------------------
