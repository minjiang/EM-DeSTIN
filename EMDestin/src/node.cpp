...
void DeSTIN(Destin * d, uint layer, float * layerSharedSigma){
	uint c, s, ns;
    float diff, learnRate, dt;
 Node * n = GetNodeFromDestin(d, layer, 0,0);
    for(c = 0 ; c < d->nb[layer]; c++){
	...
	//printf("n->layer:%d  ns:%d  (nb=%d ni=%d np=%d nc=%d winner=%d)\n",layer,ns,n->nb,n->ni,n->np,n->nc,n->winner);
        for(s = 0 ; s < ns ; s++){
            //move the centroid with the averaged delta
            dt = d->uf_avgDelta[layer][c * ns + s];
            diff = dt * learnRate;
...
//------------------------------2014.01.09 EM DeSTIN--------------
void EMDeSTIN(Node * n, uint nIdx,Destin * d){
   // printf("nIdx:%d\n",nIdx);
    n = &n[nIdx];
    n->mu=d->T[nIdx].Cluster(n->observation);//cluster on each node
}
